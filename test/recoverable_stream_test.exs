defmodule RecoverableStreamTest do
  ...
  @moduletag capture_log: true # supresses failed stream log lines
  ...
  test "number of retries" do
    ...
    assert {{:function_clause, _}, _} = catch_exit(
      RS.run(gen_stream_f())
      |> Stream.take(n)
      |> Enum.into([])
    )

    res = RS.run(gen_stream_f(), retry_attempts: 5)
          |> Stream.take(n)
          |> Enum.into([])
  ...
  test "wrapper fun" do
    ...
    res = RS.run(gen_stream_f(), wrapper_fun: wrapper, retry_attempts: 0)
          |> Stream.take(n)
          |> Enum.into([])
  ...
  test "postgrex example", %{db_conn: pid} do
    create_t = "CREATE TEMPORARY TABLE IF NOT EXISTS recoverable_stream_test" 
               <> "(a integer PRIMARY KEY, b integer, c integer)"
    inserts = "INSERT INTO recoverable_stream_test "
    ...
    assert resumable_stream_from(pid) |> Enum.count == 100000
  end
